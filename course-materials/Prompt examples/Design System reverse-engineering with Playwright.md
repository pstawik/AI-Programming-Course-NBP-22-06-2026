...
Use playwright plugin to visit a reference e-commerce website (e.g. allegro.pl, anthropic.com, or any site the group chooses) and make detailed analyses of Design Tokens, all
specific colors, fonts, margins, paddings, borders, rounded, etc. to create detailed Style Guides based on this
website, that will be later used to create new application that will match design system of the
reference website. Take screenshots and analyse them too, but also analyse DOM and styles. Prepare this document in our /docs
...
### Situation
You are working on a project to create a new application that needs to match an existing website's design system. To ensure visual consistency, you need to extract and document all design tokens and styling specifications from the reference website.

### Task
- The assistant should use the Playwright plugin to visit the reference website and conduct a comprehensive design system analysis. The assistant should:
- Navigate to the reference site and capture screenshots of key pages and components
...
### Objective
Create a comprehensive, production-ready style guide that captures the reference site's complete design system, enabling frontend developers to build a new application that achieves pixel-perfect visual consistency with the original website.
