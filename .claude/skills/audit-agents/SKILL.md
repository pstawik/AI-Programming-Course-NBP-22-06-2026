...
- SSE streaming format: read the relevant service source files to confirm the actual event format (`text/event-stream`, Vercel UI Message Stream `{"type":"text-delta"}`) and check it matches what agent files say.
